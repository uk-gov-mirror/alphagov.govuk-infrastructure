...
def handler(event:, context:)
  ...
  when 'setSecret'
    set_secret(service_client, arn, token, logger)
  when 'testSecret'
...
# Set the secret
# This method should set the AWSPENDING secret in Signon
# Should take the value of the AWSPENDING secret and set the secret to
# this value in the database.
def set_secret(service_client, arn, token, logger)
  service_client.get_secret_value(secret_id: arn, version_id: token, version_stage: 'AWSPENDING')
