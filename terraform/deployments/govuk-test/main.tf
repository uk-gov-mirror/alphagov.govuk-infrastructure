...
locals {
  ...
  mesh_domain           = "${terraform.workspace == "default" ? "${var.mesh_domain}" : "${var.mesh_subdomain}.${local.internal_domain_name}"}" #mesh.plouf.test.govuk-internal.digital
...
module "govuk" {
  ...
  public_hosted_zone_id             = var.root_public_zone_id 
