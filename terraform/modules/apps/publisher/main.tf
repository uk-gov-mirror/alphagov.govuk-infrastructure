...
resource "aws_lb" "public" {
  name               = "${terraform.workspace == "default" ? "fargate-public-${var.service_name}" : "fargate-public-${var.service_name}-${terraform.workspace}"}"
  internal           = false
...
resource "aws_lb_target_group" "public" {
  name        = "${terraform.workspace == "default" ? "${var.service_name}-public" : "${var.service_name}-${terraform.workspace}-public"}"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = var.vpc_id
  target_type = "ip"
...
resource "aws_security_group" "public_alb" {
  name        = "${terraform.workspace == "default" ? "fargate_${var.service_name}_public_alb" : "fargate_${var.service_name}_${terraform.workspace}_public_alb"}"
  vpc_id      = var.vpc_id
