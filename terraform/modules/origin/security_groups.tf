resource "aws_security_group" "origin_alb" {
  ...
  vpc_id      = var.vpc_id
  description = "${local.mode}-origin Internet-facing ALB in govuk-${var.workspace} cluster"
}

resource "aws_security_group_rule" "service_from_origin_alb_http" {
  for_each                 = var.apps_security_config_list
  description              = "${each.key} receives requests from the ${local.mode}-origin-${var.workspace} ALB over HTTP"
  type                     = "ingress"
...
# We use a secret custom header to authenticate request via the origin ALB
resource "aws_security_group_rule" "origin_alb_from_cidrs_https" {
  description       = "${local.mode}-origin-${var.workspace} ALB allows requests from CIDRs list over HTTPS"
  type              = "ingress"
  from_port         = 443
  to_port           = 443
  protocol          = "tcp"
